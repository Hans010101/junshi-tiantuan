...
知识库通过版本化 SQL 迁移管理：`advisor_domains` 保存 10 个领域，`advisors` 保存 109 人及完整 JSON 框架，`advisor_cases` 保存 451 条案例，`advisor_assets` 保存 763 条预期资源状态。图片字节作为 Workers Static Assets 发布，不放进 D1。
...
2. 补齐 545 张谋略模型图并按资源清单自动入库。
3. 增加追问阶段，而不是一次输入直接生成报告。
4. 如需账号与跨设备同步，另建用户数据域并提供导出/删除。
5. 建立匿名质量评价、提示词回归测试和安全测试集。
6. 在有明确付费需求后再接入支付、邮件和管理后台。
